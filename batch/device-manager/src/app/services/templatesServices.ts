  ...
  async remove(
    ...
    const remove_templates_all_promisses = dto.templates.map(
      async (template_id) => {
         ...
         */
        const assert_template_exists = await this.templatesRepository.findById(
          connection,
          template_id,
        );
        ...
        if (assert_template_exists) {
          ...
          if (template_to_removed != null) {
            ...
            if (qt_associated_with_devices >= 1) {
              template_to_removed[0].device_template.forEach((devices: any) => {
                if (
                  !aux_ids_device_found_associated.includes(devices.devices.id)
                ) {
